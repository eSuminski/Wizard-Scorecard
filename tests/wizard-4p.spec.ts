...
import { playerNames, playData4, wizardPath } from './fixtures';

test('4 player game full playthrough and winner validation', async ({ page }) => {
  const rounds = 15;
  ...
  for (let round = 1; round <= rounds; round++) {
    const [bBid, sBid, tBid, fBid, bTrick, sTrick, tTrick, fTrick] = playData4[round - 1];
